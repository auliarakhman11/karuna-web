...
// ─────────────────────────────────────────────
export const deleteJournalEntries = async (userId: string, referenceId: string | any): Promise<void> => {
  try {
    ...
    if (error) {
      console.warn(`[Journal] Delete failed for ref=${refId}:`, error.message);
    } else {
      console.log(`[Journal] Entries deleted for ref=${refId}`);
    }
  } catch (err: any) {
